...
import { Loop } from '#utils/FrameLoop.ts';
import { clamp, findClosestIndex, lerp as lerpFn } from '#utils/math.ts';
...
const spring = (motion: Motion, { tension: k = 170, friction: c = 26, mass: m = 1, velocity: v }: SpringConfig) => {
	...
	return function update() {
		...
		if (zeta < 1) {
			...
			value =
				motion.to -
				envelope * (((-v0 + zeta * w0 * motion.x0) / w1) * Math.sin(w1 * t) + motion.x0 * Math.cos(w1 * t));
		} else {
...
const damping = (
	motion: Motion,
	{ momentum = 0.998, snapPoints, velocity: v, min = -Infinity, max = Infinity }: DampingConfig
) => {
	const v0 = v ?? motion.startVelocity;
...
type InferConfig<T extends { easing: keyof EasingConfigs }> = T['easing'] extends infer A
	? A extends keyof EasingConfigs
		? Config<A> & EasingConfigs[A] & DampingConfig
		: never
	: never;

export class Motion<O extends number | object = any> extends Emitter<number> {
	#val: any;
	#key?: keyof O;

	constructor(valOrObject: O);
	constructor(valOrObject: O, key: keyof O);
	constructor(valOrObject: any, key?: any) {
		super();
		this.#val = valOrObject;
		this.#key = key;
	}
	...
	set = <T extends { easing: keyof EasingConfigs }>(_to: number, inputConfig?: Partial<InferConfig<T>>) => {
		const config = { ...defaultConfig, ...inputConfig } as Config<Easing> & EasingConfigs[Easing];
		...
		config.distancePrecision ??= config.snapPoints ? 0.5 : this.#easing === 'spring' ? 0.01 : 0.1;
		config.velocityPrecision ??= config.snapPoints ? 0.2 : this.#easing === 'spring' ? 0.001 : 0.01;
